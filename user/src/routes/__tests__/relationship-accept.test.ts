...
describe("accept relationship", () => {
  describe("invalid conditions", () => {
    describe("invalid id", () => {
      it("returns a 400", async () => {
        // has correct status
        await request(app)
          .post(`/api/en/user/relationships/${INVALID_ID}/accept`)
    ...
    describe("not signed in", () => {
      it("returns a 401", async () => {
        // has correct status
        await request(app)
          .post(`/api/en/user/relationships/${USER_B.id}/accept`)
          .send()
          .expect(401);
      });
    });
  ...
  describe("valid conditions", () => {
    ...
    it("should call acceptRelationship correctly", async () => {
      ...
      // has been called
      expect(acceptRelationship).toHaveBeenCalledWith(USER_A.id, USER_B.id);
    });
  });
});
