...
describe("accept relationship", () => {
  describe("invalid target user", () => {
    it("returns a 400", async () => {
      await request(app)
        .post("/api/en/user/relationships/invalid-id/accept")
  ...
  describe("non-existant target user", () => {
    it("returns a 404", async () => {
      await request(app)
        .post("/api/en/user/relationships/targettarget/accept")
        .set("Cookie", await global.signIn())
        .send()
        .expect(404);
    });
  ...
  describe("target user exists", () => {
    ...
    it("returns a 401 if not signed in", async () => {
      await request(app)
        .post(`/api/en/user/relationships/${targetUserDoc.id}/accept`)
        .send()
        .expect(401);
    });
    ...
    it("should call acceptRelationship correctly", async () => {
      await request(app)
        .post(`/api/en/user/relationships/${targetUserDoc.id}/accept`)
        .set(
          "Cookie",
      ...
      expect(acceptRelationship).toHaveBeenCalledWith(
        sourceUserDoc.id,
        targetUserDoc.id
      );
    });
