...
describe("reject relationship", () => {
  describe("invalid conditions", () => {
    describe("invalid id", () => {
      it("returns a 400", async () => {
        // has correct status
        await request(app)
          .post(`/api/en/user/relationships/${INVALID_ID}/reject`)
    ...
    describe("not signed in", () => {
      it("returns a 401", async () => {
        // has correct status
        await request(app)
          .post(`/api/en/user/relationships/${USER_B.id}/reject`)
          .send()
          .expect(401);
      });
    });
  ...
  describe("valid conditions", () => {
    ...
    it("should call rejectRelationship correctly", async () => {
      ...
      // has been called
      expect(rejectRelationship).toHaveBeenCalledWith(USER_A.id, USER_B.id);
    });
  });
});
