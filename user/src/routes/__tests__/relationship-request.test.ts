...
describe("request relationship", () => {
  describe("invalid conditions", () => {
    describe("invalid id", () => {
      it("returns a 400", async () => {
        // has correct status
        await request(app)
          .post(`/api/en/user/relationships/${INVALID_ID}/request`)
    ...
    describe("not signed in", () => {
      it("returns a 401", async () => {
        // has correct status
        await request(app)
          .post(`/api/en/user/relationships/${USER_B.id}/request`)
          .send()
          .expect(401);
      });
    });
  ...
  describe("valid conditions", () => {
    ...
    it("should call requestRelationship correctly", async () => {
      ...
      // has been called
      expect(requestRelationship).toHaveBeenCalledWith(USER_A.id, USER_B.id);
    });
  });
});
