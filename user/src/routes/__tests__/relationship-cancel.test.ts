...
describe("cancel relationship", () => {
  describe("invalid conditions", () => {
    describe("invalid id", () => {
      it("returns a 400", async () => {
        // has correct status
        await request(app)
          .post(`/api/en/user/relationships/${INVALID_ID}/cancel`)
    ...
    describe("not signed in", () => {
      it("returns a 401", async () => {
        // has correct status
        await request(app)
          .post(`/api/en/user/relationships/${USER_B.id}/cancel`)
          .send()
          .expect(401);
      });
    });
  ...
  describe("valid conditions", () => {
    ...
    it("should call cancelRelationship correctly", async () => {
      ...
      // has been called
      expect(cancelRelationship).toHaveBeenCalledWith(USER_A.id, USER_B.id);
    });
  });
});
